...
class TipologyTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ($tipologies as $tipology) {
            ...
            $new_tipology->save(); // Salva la tipologia prima di associarla

            // Seleziona la tipologia "Pizza" solo se il nome del ristorante contiene "Pizzeria"
            if ($tipology['name'] == 'Pizza') {
                $pizzaRestaurants = $restaurants->filter(function ($restaurant) {
                    return strstr($restaurant->name, 'Pizzeria');
                });

                // Associa ristoranti filtrati alla tipologia "Pizza"
                foreach ($pizzaRestaurants as $pizzaRestaurant) {
                    $new_tipology->restaurants()->attach($pizzaRestaurant->id);
                }
            } else if ($tipology['name'] == 'Italiano' || $tipology['name'] == 'Dessert' && $tipology['name'] == 'Italiano' || $tipology['name'] == 'Dessert') {
                $pizzaRestaurants = $restaurants->filter(function ($restaurant) {
                    return strstr($restaurant->name, 'Trattoria', 'Osteria');
                });
